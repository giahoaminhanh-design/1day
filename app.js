...
const SCENES=[
 {id:"guide",bg:"swings",time:"Hướng dẫn",render:(s,ss,a)=>(<>
   <h1>Ngày của em</h1>
   <p><strong>Chào mừng bạn đến với ngày của em.</strong> Đây là một ngày học đường tương tác. Mỗi lựa chọn sẽ mở ra <em>trang kể diễn biến</em> theo tên của bạn, rồi nhấn <span className="kicker">Tiếp tục</span> để đi tiếp.</p>
   <ul className="rulelist"><li>Nhạc nền phát ngay khi vào (điện thoại có thể cần chạm một cái).</li><li>FOMO tăng khi {`{name}`} bị kéo bởi ánh nhìn người khác; giảm khi {`{name}`} hiện diện, chủ động.</li><li>Cuối ngày có trang kết thúc: phân tích chỉ số + lời khuyên (Cẩm nang Bye FOMO / FOMO Buddy).</li></ul>
 ...
 {id:"home",bg:"exterior",time:"16:30",text:n=>withName(`Tan học. Hoàng hôn phủ hàng cây...`,n),choices:n=>[
...
function App(){useAudioAutoplay();const sfx=useSfx();const [s,ss]=useState({name:""});const [i,si]=useState(0);const [f,setF]=useState(0);const [out,setOut]=useState(null);const [ready,setReady]=useState(false);
 useEffect(()=>{const t=setTimeout(()=>setReady(true),40);return()=>clearTimeout(t);},[i]);
 ...
 const ending=React.useMemo(()=>{const n=s.name||"bạn";const v=f;let level,analysis,advice;
   if(v<=0){level="Không bị FOMO";analysis=`${n} giữ được nhịp riêng cả ngày. Không tách mình khỏi thế giới, chỉ chọn ở trong câu chuyện của chính mình.`;advice="Duy trì 'cửa sổ yên' buổi sáng; khi so sánh trỗi dậy, thở sâu 3 nhịp và quay về cơ thể.";}
   else if(v<1.0){level="FOMO rất nhẹ (~0.3–0.9)";analysis=`Đôi khi ${n} vẫn bị kéo bởi noti, nhưng nhận ra sớm và tự ghì nhịp lại.`;advice="Chọn 2 khoảng 'không màn hình' trong ngày (sáng sớm, trước ngủ). Đặt điện thoại xa tay 15–20 phút.";}
   else if(v<2.2){level="FOMO nhẹ";analysis=`${n} nhạy với ánh nhìn người khác (like, story). Chỉ cần 1–2 neo hiện tại là kéo mình về được.`;advice="Đọc 'Cẩm nang Bye FOMO' để hiểu vòng lặp; trò chuyện FOMO Buddy khi thấy lòng 'đói' được nhìn thấy.";}
   else if(v<3){level="FOMO trung bình";analysis=`Cảm giác sợ bỏ lỡ đã ảnh hưởng đến tập trung/tâm trạng của ${n}.`;advice="Giới hạn mạng xã hội theo khung giờ; tăng 'thân thể học': vận động ngắn, thở 4-7-8, viết 3 dòng nhật ký. Kèm Bye FOMO + FOMO Buddy.";}
   else{level="FOMO nặng";analysis=`${n} đo ngày của mình bằng ánh mắt người khác và thấy mình nhỏ dần.`;advice="Dựng lại thói quen: 'cửa sổ yên' đầu ngày, tắt push, chỉ mở mạng giờ cố định. Đọc Bye FOMO và nói chuyện FOMO Buddy.";}
   return {level,analysis,advice,value:v.toFixed(2)};},[f,s.name]);
 return (<div className={"app "+(ready?"ready":"")} style={{backgroundImage:`url(${bg})`}}><div className="card">
   <div className="topbar"><div className="badge">{scene.time}</div><div className="small">Tiến độ: {progress}% • FOMO: {f>0?`+${f}`:f}</div></div>
   {"render" in scene? scene.render(s,ss,{goto}) : out? (<div className="outcome"><h2>Kết quả lựa chọn</h2><p>{out.text}</p><div className="footer"><button className="btn" onClick={proceed}>Tiếp tục</button></div></div>) : (scene.id==="ending"? (<div className="ending"><h1>Ngày của em — Kết</h1><p><strong>Mức độ:</strong> {ending.level}</p><p><strong>Chỉ số hôm nay:</strong> {ending.value}</p><p>{ending.analysis}</p><p><strong>Lời khuyên:</strong> {ending.advice}</p><p className="small">Muốn hiểu rõ hơn về FOMO và cách quay lại với chính mình, hãy đọc <em>Cẩm nang Bye FOMO</em> hoặc trò chuyện với <strong>FOMO Buddy</strong> — người bạn lắng nghe không phán xét.</p><div className="footer"><button className="btn secondary" onClick={()=>{si(0);setF(0);ss({name:""});}}>Chơi lại</button></div></div>) : (<><h1>Ngày của em</h1><p>{typeof scene.text==="function"?scene.text(s.name):scene.text}</p><div className="choices">{scene.choices(s.name).map((c,idx)=>(<button key={idx} className="choice" onClick={()=>choose(c)}>{c.label}</button>))}</div></>))}
 </div></div>);}
